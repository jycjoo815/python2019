...
while True:
    ...
    for i in range(len(a1)):
        if  a1[i]&a2[i]&a3[i] == True: 
            ...
            disuse3.append(list(df[df['식품분류기준소분류'] == z]['세슘-137'])[i]) #index 나boolean자료형이 아닌 i번째 value 자체를 넣을것
    ...
    for i in range(len(disuse3)): #i는 물질내에 들어있는 기존 방사능 값
        i = disuse3[i]
        i = float(i)
        if i*((ma.e)**((-t*ma.log(2)/T))) > 1: #t시간이 경과한 후의 방사능 #우변은 조금 더 유동적인 값이 들어갈 수 있음.
            unsuitable.append(i)
        else:
            suitable.append(i)

    print("적합:",suitable) #출력을 index(i번 표고버섯)로 하지않음.(order가 존재)
